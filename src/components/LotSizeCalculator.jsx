  ...
  return (
    <div className="h-full">
      <Card className="bg-transparent shadow-none border-none">
        <CardHeader className="p-3 pt-2 pb-1 space-y-1 relative">
        ...
        <CardContent className="p-3 pt-0 space-y-1.5">
          ...
          <div className="inline-flex items-center bg-gray-100 dark:bg-gray-800/60 rounded-full p-1 border border-gray-200 dark:border-gray-700">
            {Object.entries(instrumentConfigs).map(([key, config], idx) => (
              <button
                ...
                } px-2.5 py-1 text-xs font-medium rounded-full transition-colors duration-200 ${idx !== 0 ? 'ml-1' : ''}`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-1.5">
            {/* Account Balance */}
            <div>
              <label htmlFor="accountBalance" className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-0.5">
                Account Balance ($)
              </label>
              <input
                id="accountBalance"
                ...
                className={`w-full px-2.5 py-1.5 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white ${
                  errors.accountBalance ? 'border-red-500' : 'border-gray-300'
                }`}
                placeholder="10000"
              />
              {errors.accountBalance && (
                <p className="text-red-500 text-xs mt-0.5">{errors.accountBalance}</p>
              )}
            </div>

            {/* Risk Percentage */}
            <div>
              <label htmlFor="riskPercentage" className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-0.5">
                Risk Percentage (%)
              </label>
              <input
                id="riskPercentage"
              ...
              {errors.riskPercentage && (
                <p className="text-red-500 text-xs mt-0.5">{errors.riskPercentage}</p>
              )}
            </div>

            {/* Currency Pair Selection */}
            <div>
              <label htmlFor="currencyPair" className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-0.5">
              ...
              <select
                ...
                className="w-full px-2.5 py-1.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            ...
            {/* Stop Loss */}
            <div>
              <label htmlFor="stopLoss" className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-0.5">
              ...
              />
              {errors.stopLoss && (
                <p className="text-red-500 text-xs mt-0.5">{errors.stopLoss}</p>
              )}
            </div>
            ...
            {(formData.instrumentType === 'crypto' || formData.instrumentType === 'commodities') && (
              <div className="md:col-span-2">
                <label htmlFor="currentPrice" className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-0.5">
                  Current Price ($)
                  {isConnected && formData.currentPrice && (
                    <span className="ml-2 inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400">
                      Auto
                    </span>
                  )}
                </label>
                <div className="relative">
                  <input
                    ...
                    className={`w-full px-2.5 py-1.5 pr-8 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white ${
                  ...
                  {isConnected && formData.currentPrice && (
                    <button
                      ...
                      className="absolute right-2 top-1/2 transform -translate-y-1/2 p-0.5 text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300"
                ...
                {errors.currentPrice && (
                  <p className="text-red-500 text-xs mt-0.5">{errors.currentPrice}</p>
                )}
                {isConnected && formData.currentPrice && (
          ...
          <div className="flex gap-1.5">
            <Button
              ...
              className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-1.5 px-3 rounded-lg transition-colors duration-200 text-sm"
            >
              Calculate Lot Size
            </Button>
            <Button
              ...
              variant="outline"
              className="px-4 py-1.5 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200 text-sm"
            >
              Reset
            </Button>
          </div>
          ...
          {/* Results */}
          {result && (
            <div className="mt-3 p-2.5 bg-gradient-to-r from-green-50 to-blue-50 dark:from-green-900/20 dark:to-blue-900/20 rounded-lg border border-green-200 dark:border-green-800">
              <h3 className="text-base font-bold text-gray-900 dark:text-white mb-1.5 flex items-center">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-1.5">
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-gray-600 dark:text-gray-300">Risk Amount:</span>
                    <span className="font-semibold text-red-600">${result.riskAmount.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-gray-600 dark:text-gray-300">Position Size:</span>
                ...
                <div className="space-y-1">
                  <div className="text-xs text-gray-600 dark:text-gray-300">
                    ...
                    <div className="mt-0.5 p-1.5 bg-white dark:bg-gray-800 rounded border font-mono text-[11px]">
                      {result.calculation}
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-1.5 p-1.5 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
                <h4 className="font-medium text-yellow-800 dark:text-yellow-200 mb-0.5 text-xs">💡 Risk Tips:</h4>
