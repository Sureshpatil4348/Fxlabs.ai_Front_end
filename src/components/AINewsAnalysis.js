...
const CountdownTimer = ({ newsItem, className = "" }) => {
    ...
    if (isExpired) {
        return (
            <span
                className={`text-xs font-medium text-gray-600 dark:text-gray-400 ${className}`}
            >
                Started
            </span>
        );
    ...
    return (
        <span
            className={`text-xs font-medium text-orange-600 dark:text-orange-400 ${className}`}
        >
            {timeLeft}
        </span>
    );
...
const NewsModal = ({ news, analysis, isOpen, onClose }) => {
    ...
    const { time, date } = formatNewsLocalDateTime({
        dateIso: news.date,
        originalTime: news.originalTime,
    });
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg max-w-2xl w-full max-h-[80vh] overflow-y-auto">
                ...
                <div className="flex items-center justify-between py-2 px-4 border-b dark:border-slate-600">
                    <div className="flex items-center space-x-2">
                        <span className="text-sm font-bold px-2 py-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 rounded">
                            {news.currency}
                        </span>
                        ...
                        <div>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-slate-700 rounded-lg">
                                    <span className="text-gray-700 dark:text-slate-300 font-medium text-sm">
                                        Expected Effect:
                                    </span>
                                    <div className="flex items-center space-x-2">
                                ...
                                {analysis.suggestedPairs &&
                                    analysis.suggestedPairs.length > 0 && (
                                        <div className="p-3 bg-gray-50 dark:bg-slate-700 rounded-lg">
                                            <div className="flex flex-wrap gap-1.5">
                                                {analysis.suggestedPairs.map(
                                                    (pair) => (
                                                        <span
                                                            key={pair}
                                ...
                                <div className="p-3 bg-gray-50 dark:bg-slate-700 rounded-lg">
                                    <div className="text-gray-700 dark:text-slate-300 font-medium mb-2 text-sm">
                                        Impacted Currency:
                                    </div>
                                    <div className="flex flex-wrap gap-1.5">
                                        {analysis.impactedCurrencies.map(
                                            (currency) => {
                                                const info =
                                                ...
                                                return (
                                                    <span
                                                        key={currency}
                                                        className="flex items-center space-x-2 px-2.5 py-1.5 bg-white dark:bg-slate-600 rounded-lg border dark:border-slate-500"
                                                    >
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            <div className="text-center p-2 bg-gray-50 dark:bg-slate-700 rounded-lg">
                                <div className="text-gray-600 dark:text-slate-400 mb-1 text-sm">
                                    Previous
                                </div>
...
const NewsCard = ({ news, analysis, onShowDetails }) => {
    ...
    const borderClass =
        effect === "Bullish"
            ? "border-success-600 border-2"
            : effect === "Bearish"
            ? "border-danger-300 border-2"
            ...
            <div className="flex items-start justify-between mb-2">
                <div className="flex-1">
                    <div className="flex items-center space-x-2 mb-1">
                        <span className="text-xs font-bold px-2 py-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 rounded">
                            {news.currency}
                        </span>
                        <span
                    ...
                    <div className="mb-2">
                        <div className="flex flex-wrap items-center gap-0.5 max-w-full overflow-hidden">
                            ...
                            {analysis.suggestedPairs.length > 5 && (
                                <span className="text-blue-600 dark:text-blue-400 font-bold text-xs sm:text-base ml-0.5 flex-shrink-0">
                                    ...
                                </span>
                            )}
                        </div>
...
const AINewsAnalysis = () => {
    const { newsData, aiAnalysis, newsLoading } = useBaseMarketStore();

    // Get tab state from base market store
    const { tabState, tabStateHasLoaded, updateNewsFilter } =
        useBaseMarketStore();

    const [newsFilter, setNewsFilter] = useState(
        tabState.news?.filter || "upcoming"
    );
    ...
    const allNews = newsData
        ...
        .filter((n) =>
            typeof n.impact === "string"
                ? n.impact.toLowerCase() === "high"
                : false
        );
    ...
    useEffect(() => {
        try {
            ...
            console.log("AI News: raw fetched news", {
                count: newsData?.length ?? 0,
                items: newsData,
            });

            const todays = (newsData || []).filter(isNewsTodayLocal);
            const highImpactToday = todays.filter((n) =>
                typeof n.impact === "string"
                    ? n.impact.toLowerCase() === "high"
                    : false
            );
            ...
            // eslint-disable-next-line no-console
            console.log("AI News: today (all impacts)", {
                count: todays.length,
                items: todays,
            });
            // eslint-disable-next-line no-console
            console.log("AI News: today (HIGH impact)", {
                count: highImpactToday.length,
                items: highImpactToday,
            });
            ...
            // eslint-disable-next-line no-console
            console.log("AI News: filtered.upcoming (today HIGH)", {
                count: upcoming.length,
                items: upcoming,
            });
            // eslint-disable-next-line no-console
            console.log("AI News: filtered.released (today HIGH)", {
    ...
    const sortedNews = filteredNews.sort((a, b) => {
        const aTs = formatNewsLocalDateTime({
            dateIso: a.date,
            originalTime: a.originalTime,
        }).dateObj.getTime();
        const bTs = formatNewsLocalDateTime({
            dateIso: b.date,
    ...
    const handleFilterChange = async (filter) => {
        ...
        try {
            if (typeof window !== "undefined")
                window.localStorage.setItem("ainews_filter", filter);
        } catch (e) {}
        try {
        ...
        <div className="widget-card px-3 pb-2 z-1 relative h-full flex flex-col">
                ...
                <div className="flex space-x-0.5 mb-1 p-0.5 bg-gray-100 dark:bg-slate-700 rounded-lg text-[12px]">
                    {filters.map((filterOption) => (
                        ...
                        >
                            {filterOption.label}
                            <span
                                className={`ml-0.5 px-1 py-0.5 rounded-full text-[10px] ${
                                    newsFilter === filterOption.id
                                        ? "bg-gray-100 dark:bg-slate-500 text-gray-700 dark:text-slate-200"
            ...
            <div
                className={`flex-1 min-h-0 p-1.5 ${
                    hasNews ? "overflow-y-auto" : "overflow-visible"
                }`}
            >
                {/* News Feed */}
