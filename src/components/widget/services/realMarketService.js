...
export class RealMarketService {
     ...
     */
    async fetchOhlcSlice(symbol = 'EURUSD', interval = '1m', { limit = 150, before = null, after = null } = {}) {
      try {
    ...
    async getHistoricalData(
      ...
      const res = await this.fetchOhlcSlice(symbol, interval, { limit: Math.min(limit || 150, 1000) });
      return res.candles;
    }
  ...
  async getHistoricalDataWithPage(
    ...
    const res = await this.fetchOhlcSlice(symbol, interval, { limit: Math.min(perPage || 150, 1000) });
