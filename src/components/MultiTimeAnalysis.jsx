...
import {
    computeMarketHours,
    listTimezonesWithOffsets,
} from "../utils/marketHoursEngine";
...
const ForexMarketTimeZone = () => {
    ...
    useEffect(() => {
        const handler = () =>
            requestAnimationFrame(() => updateIndicatorLeft());
    ...
    useEffect(() => {
        if (isDropdownOpen && tzInputRef.current) {
            ...
            setTimeout(
                () => tzInputRef.current && tzInputRef.current.focus(),
                0
            );
        }
    }, [isDropdownOpen]);
    ...
    const getMarketStatus = (timezone) => {
        const result = computeMarketHours({
            viewInstantUTC: new Date(),
            viewerTz: selectedTimezone,
        });
        const map = new Map(
            result.sessions.map((s) => [s.cityTzLabel, s.isOpenNow])
        );
    ...
    const timezoneOptions = React.useMemo(
        () =>
            listTimezonesWithOffsets(new Date()).map((item) => ({
                value: item.value,
                label: item.label,
                flag: item.flag,
                gmt: item.gmt,
    ...
    const currentTimezoneOption = React.useMemo(() => {
        ...
        return (
            timezoneOptions[0] || { label: "UTC", gmt: "+00:00", value: "UTC" }
        );
    ...
    const marketData = React.useMemo(() => {
        return computeMarketHours({
            viewInstantUTC: currentTime,
            viewerTz: selectedTimezone,
        });
    ...
    const getSessionSegments = (marketTimezone) => {
        const session = marketData.sessions.find(
            (s) => s.cityTzLabel === marketTimezone
        );
        if (!session) return [];
        if (Array.isArray(session.projectedSegmentsInViewer))
            return session.projectedSegmentsInViewer;
        if (session.projectedSegmentInViewer)
            return [session.projectedSegmentInViewer];
        return [];
    };
    ...
    const labelForSegment = (segments, idx) => {
        ...
        if (segments.length === 2) {
            const durations = segments.map((s) =>
                getDurationHoursFromISO(s.startLocalISO, s.endLocalISO)
            );
            ...
            if (minDur < 2.5) {
                ...
                let eveningIdx = segments.findIndex((s) =>
                    isMidnightLocal(s.endLocalISO)
                );
                if (eveningIdx === -1) {
                    // fallback: later start time is evening part
                    eveningIdx =
                        getTimeFloatFromISO(segments[0].startLocalISO) >
                        getTimeFloatFromISO(segments[1].startLocalISO)
                            ? 0
                            : 1;
                }
                const morningIdx = eveningIdx === 0 ? 1 : 0;
                const fullLabel = formatRangeFromISO(
                    segments[eveningIdx].startLocalISO,
                    segments[morningIdx].endLocalISO
                );
        ...
        if (startIsMidnight && !endIsMidnight)
            return formatTimeFromISO(seg.endLocalISO);
        if (!startIsMidnight && endIsMidnight)
            return formatTimeFromISO(seg.startLocalISO);
    ...
    return (
        <div className="bg-white dark:bg-[#19235d] p-2 sm:p-3 max-w-4xl mx-auto font-sans relative rounded-xl shadow-none dark:shadow-none border-0">
            ...
            <div className="absolute top-3 sm:top-4 right-3 sm:right-4 flex items-center gap-2 z-30">
                                ...
                                >
                                    <div className="flex items-center gap-2">
                                        <div className="flex items-center justify-center w-10 h-10 rounded-lg border-2 border-gray-300 dark:border-[#19235d] bg-white dark:bg-[#19235d]">
                                            <span className="text-[#19235d] dark:text-gray-300 text-[10px] font-bold">
                                                {m.currency}
                                            </span>
                                        </div>
                                        <div className="min-w-0">
                                            <h3 className="font-bold text-xs text-[#19235d] dark:text-white truncate">
                                                {m.name}
                                            </h3>
                                            <p className="text-[10px] text-[#19235d] dark:text-gray-200 leading-tight">
                                                {formatTime(
                                                    currentTime,
                                                    m.timezone
                                                )}
                                            </p>
                                            <p className="text-[10px] text-[#19235d] dark:text-gray-200 leading-tight truncate">
                                                {formatDate(
                                                    currentTime,
                                                    m.timezone
                                                )}
                            ...
                            {!marketData.viewer.retailGateOpen && (
                                <div className="mt-2 sm:mt-3 lg:mt-1 mb-1 sm:mb-2">
                                    <div className="hidden lg:flex items-center mx-3 sm:mx-4 lg:mx-0">
                                        <div className="w-48 lg:w-56"></div>{" "}
                                        {/* Spacer for desktop */}
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-3 w-48 lg:w-56 flex-shrink-0 px-3 sm:px-4">
                                            <div className="flex items-center justify-center w-12 h-12 rounded-lg border-2 border-gray-300 dark:border-[#19235d] bg-white dark:bg-[#19235d]">
                                                <span className="text-[#19235d] dark:text-gray-300 text-xs font-bold">
                                                    {m.currency}
                                                </span>
                                            </div>
                                            <div>
                                                <h3 className="font-bold text-sm text-[#19235d] dark:text-white">
                                                    {m.name}
                                                </h3>
                                                <p className="text-xs text-[#19235d] dark:text-gray-200">
                                                    {formatTime(
                                                        currentTime,
                                                        m.timezone
                                                    )}
