import { Bell, Sliders as _Sliders } from "lucide-react";
...
const MultiIndicatorHeatmap = ({ selectedSymbol = "EURUSDm" }) => {
    ...
    const [showIndicatorAlertConfig, setShowIndicatorAlertConfig] =
        useState(false);
        ...
        const loadSettings = async () => {
            try {
                ...
                if (savedState && Object.keys(savedState).length > 0) {
                    ...
                    const normalizedStyle = allowedStyles.includes(style)
                        ? style
                        : "swingTrader";
                    ...
                    _setShowNewSignals(
                        showNewSignals !== undefined ? showNewSignals : true
                    );
                }
            } catch (error) {
                console.error(
                    "Failed to load MultiIndicatorHeatmap state:",
                    error
                );
            }
        };
    ...
    const handleAlertConfigClose = () => {
        ...
        if (user) {
            const loadActiveAlertsCount = async () => {
                try {
                    const alert =
                        await heatmapTrackerAlertService.getActiveAlert();
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            const currentSubscriptions = Array.from(
                useRSITrackerStore.getState().subscriptions.keys()
            );
    ...
    useEffect(() => {
        ...
        const supported =
            Array.isArray(timeframes) && timeframes.length > 0
                ? timeframes
                : desiredTimeframes;
        const toSubscribe = desiredTimeframes.filter((tf) =>
                ...
                <div className="mb-0.5 px-2">
                    ...
                    <div className="widget-header flex flex-wrap items-center justify-between gap-1 mb-0.5">
                        ...
                        <div className="flex items-center shrink-0">
                            ...
                            />
                            <CardTitle className="text-lg font-bold text-gray-900 dark:text-white tools-heading">
                                Quantum Analysis
                            </CardTitle>
                        </div>
                                ...
                                >
                                    <div className="relative">
                                        ...
                                        <div className="absolute inset-y-0 right-0 flex items-center pr-1 pointer-events-none">
                                            <svg
                                                className={`w-2 h-2 text-gray-500 dark:text-slate-400 transition-transform duration-200 ${
                                                    isSymbolDropdownOpen
                                                        ? "rotate-180"
                                                        : ""
                                                }`}
                                                    ...
                                                    (option) => (
                                                        <button
                                                            ...
                                                            className={`w-full text-left px-3 py-2 text-sm hover:bg-blue-50 dark:hover:bg-slate-700 transition-colors duration-150 ${
                                                                option.value ===
                                                                currentSymbol
                                                                    ? "bg-blue-100 dark:bg-slate-600 text-blue-800 dark:text-slate-200 font-medium"
                        ...
                        <div className="flex-1 min-w-0 overflow-x-auto overflow-y-visible">
                            <table className="w-full border-collapse min-w-[560px]">
                                <thead>
                                    <tr className="border-b-0">
                                        ...
                                        {indicators.map((indicator) => (
                                            <th
                                                key={indicator}
                                                className="text-center py-0.5 px-0.5 text-gray-800 dark:text-gray-200"
                                            >
                                ...
                                <tbody>
                                    {(() => {
                                        const mcState =
                                            useMarketCacheStore.getState();
                                        const supportedTfs =
                                            Array.isArray(
                                                mcState.supportedTimeframes
                                            ) &&
                                            mcState.supportedTimeframes.length >
                                                0
                                                ? mcState.supportedTimeframes
                                                : [
                                                      "1M",
                                        ...
                                        const perTf =
                                            (
                                                mcState.quantumBySymbol.get(
                                                    currentSymbol
                                                ) || {}
                                            ).per_timeframe || {};
                                        return [...new Set(supportedTfs)]
                                            .filter((tf) => tf !== "1W")
                                            .map((timeframe) => (
                                                <tr
                                                    key={timeframe}
                                                    className="border-b border-slate-100/50 dark:border-slate-700/50"
                                                >
                            ...
                            {(() => {
                                ...
                                const styleKey =
                                    tradingStyle === "swingTrader"
                                        ...
                                        : "scalper";
                                const qe =
                                    mcState.quantumBySymbol.get(currentSymbol);
                                const ov =
                                    qe && qe.overall
                                        ? qe.overall[styleKey]
                                        : null;
                                const buyPct =
                                    typeof ov?.buy_percent === "number"
                                        ? ov.buy_percent
                                        : 50;
                                const sellPct =
                                    typeof ov?.sell_percent === "number"
                                        ? ov.sell_percent
                                        : 100 - buyPct;
                                            ...
                                            >
                                                ...
                                                {Array.from({ length: 11 }).map(
                                                    (_, i) => {
                                                        ...
                                                        const ang =
                                                            -Math.PI / 2 +
                                                            Math.PI * t; // -90deg .. +90deg
                                                        const inner = 82; // inner radius for tick start
                                                        const outer =
                                                            i % 5 === 0
                                                                ? 94
                                                                : 90; // longer for major ticks
                                                        const x1 =
                                                            100 +
                                                            Math.cos(ang) *
                                                                inner;
