...
const workflow = [
  {
    title: 'Detect',
    description: '7-indicator core scans TradingView conditions for confluence across M1 to H4 timeframes, driven by artificial intelligence for forex trading.',
    icon: Zap
  },
  {
    title: 'Filter',
    description: 'High-impact calendar events and liquidity windows are cross-checked to avoid false triggers, a smart feature of the best forex trading platform.',
    icon: Server
  },
  {
    title: 'Notify',
    description: 'Instant alerts route to email, mobile and desktop so your team never misses a signal from your chosen AI forex trading software.',
    icon: MailCheck
  },
  {
    title: 'Act',
    description: 'Automated summaries include entry zones, stop-loss guidance and current currency strength, providing actionable data from a leading Top forex trading platform.',
    icon: Inbox
  }
]
        ...
        <div className="grid lg:grid-cols-2 gap-8 sm:gap-10 xl:gap-16 items-center">
          <div className="space-y-4 sm:space-y-6 text-center lg:text-left">
            ...
            <p className="text-sm sm:text-base md:text-lg text-[#19235d]/80 leading-relaxed" style={{ fontFamily: 'Pier Sans, sans-serif' }}>
              FxLabs Prime watches global markets around the clock, combining news, indicators and currency strength to deliver precise trade plans right when you need them. This advanced monitoring is a key feature of the best forex trading platform. No more refreshing screens or missing volatility.
            </p>
            <ul className="space-y-3 sm:space-y-4 text-[#19235d]/80" style={{ fontFamily: 'Pier Sans, sans-serif' }}>
              <li className="flex flex-col sm:flex-row sm:items-start sm:justify-center lg:justify-start text-center sm:text-left">
                ...
                <span className="mt-3 sm:mt-0">High-impact news, indicator confluence and volatility alerts delivered to email, SMS and webhook—crucial for any Top forex trading platform.</span>
              </li>
              <li className="flex flex-col sm:flex-row sm:items-start sm:justify-center lg:justify-start text-center sm:text-left">
                <Inbox className="w-5 h-5 text-emerald-500 mx-auto sm:mx-0 sm:mt-0.5 sm:mr-3" />
                <span className="mt-3 sm:mt-0">Custom routing by asset class or session so your team receives only what matters, demonstrating the flexibility of advanced AI-based best forex trading platforms.</span>
              </li>
              <li className="flex flex-col sm:flex-row sm:items-start sm:justify-center lg:justify-start text-center sm:text-left">
                <Server className="w-5 h-5 text-emerald-500 mx-auto sm:mx-0 sm:mt-0.5 sm:mr-3" />
                <span className="mt-3 sm:mt-0">Secure Dubai-hosted infrastructure with redundancy for uninterrupted monitoring, a standard for reliable AI forex trading software.</span>
              </li>
            </ul>
          </div>
              ...
              {workflow.map(({ title, description, icon: Icon }) => (
                <div key={title} className="rounded-2xl sm:rounded-3xl bg-white/60 backdrop-blur border border-white/40 p-4 sm:p-6 shadow-xl">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-gradient-to-br from-emerald-100 to-green-200 flex items-center justify-center flex-shrink-0">
