...
CREATE POLICY "Users read own heatmap_tracker_alert_triggers" ON public.heatmap_tracker_alert_triggers
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.heatmap_tracker_alerts a
      WHERE a.id = heatmap_tracker_alert_triggers.alert_id AND a.user_id = auth.uid()
    )
  );

CREATE POLICY "Users insert own heatmap_tracker_alert_triggers" ON public.heatmap_tracker_alert_triggers
